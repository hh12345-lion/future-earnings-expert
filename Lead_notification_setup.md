follow # Lead notification webhook (n8n)
...
## This project (Future Earnings Expert)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `Future Earnings Expert` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL` is set) | `futureearningsexpert.com` |
| Contact page form | Instruct-style intake; webhook **`/api/submit-lead`** with **`formType: "instruct"`** (primary) |
| Google Sheets | Soft-fail via **`/api/instruct`** → one tab **`GOOGLE_SHEET_TAB_NAME`**, **Form Type** = `Instruct` |
| Email | Soft-fail (no Resend); intake logged server-side only |
